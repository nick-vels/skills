...
`computeRouting` carries the last session's diagnostic capabilities and current abstract effort band, while each ticket's
...
those capabilities. Provider-specific model IDs do not belong in portable run state.
