...
2. Read `phases/0-compute.md` and re-probe routing capabilities for this caller before the first routing-dependent action. Persisted capability fields are diagnostic, not proof for a resumed session. If `pendingEffortTransition` is non-null, resolve its distinct-request confirmation before product work.
...
4. A ticket marked `in-progress` in `state.js` with no commit behind it was interrupted mid-flight. Reset it to `pending` and run it again from scratch — a half-applied ticket is worse than a fresh one.
...
5. Re-run the Phase 6 checklist over the whole diff since the last green commit before continuing. Something may have been left broken.
6. Continue from the frontier. Do not redo finished phases; do not re-ask answered questions.
