...
Третья ручка, и единственная, которая стоит заметных денег и времени. **По умолчанию выключена.**

```
/autopilot polish лендинг студии маникюра
```

Включается словом `polish` или «вылижи», «доведи до идеала», «сравни с эталоном». Что происходит: когда проект уже собран и принят, агент запускает его, ставит результат рядом с **вашим** эталоном и ищет конкретные различия. Найденное становится обычными тасками — с ревью, тестами и отдельными коммитами, — и круг повторяется.

**Эталон — это то, что дали вы.** Сайты, на которые должно быть похоже; скриншот; текст, тон которого вам нравится; число, которое надо побить. Всё это агент спросит один раз в брифинге и сложит в `reference.md`.

Без эталона доводка **не запускается** — и это главное в ней. Критику, которому не с чем сравнивать, приходится придумать эталон самому, и дальше сборка честно едет к стандарту, которого никто не выбирал. Поэтому если сравнивать не с чем, агент скажет об этом одной строкой и пойдёт к отчёту.

Три способа остановиться, срабатывает первый попавшийся: круг не нашёл ничего нового, пройдено три круга, или вы сказали «хватит». «Пока критик не будет доволен» — не условие: он не будет доволен никогда, это его работа. И круг, который что-то сломал, откатывается целиком, а не чинится следующим кругом.

Уместна доводка не везде. Там, где качество видно и есть с чем сравнить — интерфейсы, лендинги, тексты, — она даёт больше всего. Для внутренней логики с внятной спецификацией судья и так есть: это тесты.
...
│   ├── 2-briefing.md           вопросы пользователю, сбор эталона
│   ├── 3-spec.md               спецификация и проработка глубины
│   ├── 4-plan.md               разбивка на таски, ярусы
│   ├── 5-compute.md            model/effort для каждого executor по риску таска
│   ├── 5-subagents.md          агенты-исполнители и контракты между ними
│   ├── 6-review.md             проверка по трём осям
│   ├── 7-instruments.md        состояние и дашборд — читается при нарезке тасков
...
Единица подгрузки — файл, а не раздел: прочитать «только верхушку» нельзя, читается всё. Поэтому то, что нужно фазе 0 от дашборда и от памяти проекта, вынесено в `0-instruments.md` и `0-memory.md` — иначе старт сборки тянул бы за собой два больших файла ради нескольких абзацев. По той же причине `polish.md` не открывается, пока доводку не попросили, а `prompts/` уходит субагенту по пути и в контексте оркестратора не оседает.
